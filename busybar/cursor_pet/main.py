...
from busybar.common.backend import BusyBarBackend, DisplayBackend
from busybar.common.input_events import InputButton, InputEvent, InputEventPump
from busybar.common.preflight import check_device, format_unreachable_help, is_host_unreachable
from busybar.common.simulator import TerminalBackend
from busybar.cursor_pet.achievements import evaluate_achievements
from busybar.cursor_pet.collector import AccountRole, UsageCollector, UsageSnapshot
from busybar.cursor_pet.notify import notify_events
from busybar.cursor_pet.pet import APP_NAME, PetScene
from busybar.cursor_pet.state import load_or_create, reroll, save_state
from busybar.cursor_pet.tokens import DEFAULT_TOKENS_PATH, resolve_collectors
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Dual-account token store (export via python -m busybar.cursor_pet.export_token)",
    )
...
def make_handler(runtime: PetRuntime) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            from busybar.cursor_pet.collector import compute_delta
...
def collector_loop(runtime: PetRuntime) -> None:
    ...
    if not token_map:
        print(
            ...
            "  python -m busybar.cursor_pet.export_token --account personal --verify\n"
            "  python -m busybar.cursor_pet.export_token --account work --verify"
        )
        return
    collectors = [
